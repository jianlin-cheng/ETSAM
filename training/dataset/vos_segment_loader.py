...
class MultipleNumpySegmentLoader:
    def __init__(self, video_png_root, single_object_mode=False):
        """
        video_png_root: the folder contains all the masks stored in npy
        single_object_mode: whether to load only a single object at a time
        """
        self.video_png_root = video_png_root
        self.single_object_mode = single_object_mode
        # read a mask to know the resolution of the video
        if self.single_object_mode:
            tmp_mask_path = glob.glob(os.path.join(video_png_root, "*.npy"))[0]
        else:
            tmp_mask_path = glob.glob(os.path.join(video_png_root, "*", "*.npy"))[0]
        tmp_mask = np.load(tmp_mask_path)
    ...
    def _load_single_png(self, frame_id):
        """
        load single npy from the disk (path: f'{self.obj_id}/{frame_id:05d}.npy')
        Args:
            frame_id: int, define the mask path
        Return:
            binary_segments: dict
        """
        mask_path = os.path.join(self.video_png_root, f"{frame_id:05d}.npy")
        binary_segments = {}
    ...
    def _load_multiple_pngs(self, frame_id):
        """
        load multiple npy masks from the disk (path: f'{obj_id}/{frame_id:05d}.npy')
        Args:
            frame_id: int, define the mask path
        Return:
            binary_segments: dict
        ...
        for obj_folder in all_objects:
            ...
            obj_id = int(obj_folder.split("/")[-1])
            obj_id = obj_id + 1  # offset 1 as bg is 0
            mask_path = os.path.join(obj_folder, f"{frame_id:05d}.npy")
            if os.path.exists(mask_path):
                mask = np.load(mask_path)
            else:
                mask = np.zeros((self.H, self.W), dtype=bool)
