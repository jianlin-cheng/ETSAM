...
class NumpyRawDatasetWithPrompt(VOSRawDataset):
    def __init__(
        self,
        img_folder,
        gt_folder,
    ...
    def get_video(self, idx):
        ...
        segment_loader = MultipleNumpySegmentLoader(
            video_mask_root, self.single_object_mode
        )

        prompt_mask_root = os.path.join(self.prompt_folder, video_name)

        prompt_segment_loader = MultipleNumpySegmentLoader(
            prompt_mask_root, self.single_object_mode
        )

        all_frames = sorted(glob.glob(os.path.join(video_frame_root, "*.npy")))
        if self.truncate_video > 0:
            all_frames = all_frames[: self.truncate_video]
        frames = []
        for _, fpath in enumerate(all_frames[:: self.sample_rate]):
            ...
            frames.append(VOSFrame(fid, image_path=fpath))
        video = VOSVideo(video_name, idx, frames)
        return video, segment_loader, prompt_segment_loader

    def __len__(self):
        return len(self.video_names)


class NumpyRawDataset(VOSRawDataset):
    def __init__(
        self,
        img_folder,
        gt_folder,
