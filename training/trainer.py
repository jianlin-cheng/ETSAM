...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        self.where = 0.0
        # self.custom_prompting = custom_prompting
    ...
    def _step(
        ...
        model: nn.Module,
        phase: str,
        custom_input_prompt_masks: torch.Tensor = None,
    ):

        if custom_input_prompt_masks is not None:
            targets = batch.masks.clone()
            batch.masks = custom_input_prompt_masks
            outputs = model(batch)
        else:
            outputs = model(batch)
            targets = batch.masks
    ...
    def create_custom_input_prompt_masks(self, ground_truth_mask, predicted_mask, data_iter):
        # print(ground_truth_mask.shape)
        # print(ground_truth_mask.dtype)
        if torch.all(ground_truth_mask == 0):
            print(f"WRONG: ground_truth_mask - all zeros at data_iter {data_iter}!!!!!!")

        ground_truth_mask = ground_truth_mask.squeeze()#.cpu().numpy()
        stacked_predicted_masks = []
        for i in range(len(predicted_mask)):
            stacked_predicted_masks.append((predicted_mask[i]['pred_masks_high_res'] > 0).int())
        stacked_predicted_masks = torch.stack(stacked_predicted_masks, dim=0)
        
        if torch.all(stacked_predicted_masks == 0):
            print(f"WRONG: predicted_mask - all zeros at data_iter {data_iter}!!!!!!")

        stacked_predicted_masks = stacked_predicted_masks.squeeze().cpu().numpy()

        # expand the predicted mask by 3 voxels
        expanded_predicted_mask = np.zeros_like(stacked_predicted_masks)
        for i in range(stacked_predicted_masks.shape[0]):
            expanded_predicted_mask[i] = expand_labels(stacked_predicted_masks[i], distance=3)

        # with mrcfile.new(f"/data/predicted_mask_{data_iter}.mrc", overwrite=True) as mrc:
        #     mrc.set_data(expanded_predicted_mask.astype(np.uint8))
        #     mrc.voxel_size = 10

        ground_truth_mask[expanded_predicted_mask >= 1] = 0
        ground_truth_mask = ground_truth_mask.unsqueeze(1)

        del expanded_predicted_mask
        del stacked_predicted_masks

        # print(custom_input_prompt_masks.shape)
        # print(custom_input_prompt_masks.dtype)

        return ground_truth_mask


        # for i in range(gt_mask.shape[0]):
        #     # Image: Handle any shape by ensuring it's in (H, W, C) format
        #     img = batch.img_batch[i].cpu().numpy()
        #     if img.shape[0] == 3:  # If channels first (C, H, W)
        #         img = np.transpose(img, (1, 2, 0))  # Convert to (H, W, C)
        #     img = (img * 255).astype(np.uint8)

        #     # Mask: Remove batch dimension and convert to uint8
        #     mask = batch.masks[i].int().cpu().numpy()
        #     if len(mask.shape) > 2:  # If there's an extra dimension
        #         mask = mask.squeeze()
        #     mask = (mask * 255).astype(np.uint8)

        #     # Predicted mask: Remove batch and channel dimensions
        #     pred_mask = (pred_masks[i]['pred_masks_high_res'] > 0).int().cpu().numpy()
        #     if len(pred_mask.shape) > 2:  # If there's an extra dimension
        ...
        #     cv2.imwrite(f"/data/img_{i}.png", img)
        #     cv2.imwrite(f"/data/mask_{i}.png", mask)
        #     cv2.imwrite(f"/data/pred_mask_{i}.png", pred_mask)
        #     print(f"img_{i}.png, mask_{i}.png, pred_mask_{i}.png saved")
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                # if self.custom_prompting:
                #     self.model.eval()
                #     gt_masks = batch.masks.clone()
                #     # use empty mask when evaluating
                #     batch.masks = torch.zeros_like(batch.masks)
                #     with torch.no_grad():
                #         current_pred_masks = self.model(batch)
                    
                #     custom_input_prompt_masks = self.create_custom_input_prompt_masks(gt_masks.clone(), current_pred_masks, data_iter)
                #     del current_pred_masks

                #     # torch check if tensor is all zeros
                #     if torch.all(custom_input_prompt_masks == 0):
                #         print(f"Skip - custom_input_prompt_masks - all zeros at data_iter {data_iter}")
                #         continue
                        
                #     print("CUSTOM INPUT PROMPT MASKS AVAILABLE - data_iter: ", data_iter, "/", len(train_loader), "!!!!!!!")
                #     # with mrcfile.new(f"/data/ground_truth_mask_{data_iter}.mrc", overwrite=True) as mrc:
                #     #     # Convert to uint8 before saving to MRC
                #     #     mask_data = custom_input_prompt_masks.squeeze().cpu().numpy().astype(np.uint8)
                ...
                #     self.model.train()
                #     self._run_step(batch, phase, loss_mts, extra_loss_mts, custom_input_prompt_masks=custom_input_prompt_masks)
                # else:
    ...
    def _run_step(
        ...
        custom_input_prompt_masks_for_train: torch.Tensor = None,
    ):
        """
        ...
        with torch.cuda.amp.autocast(
            ...
            loss_dict, batch_size, extra_losses = self._step(
                ...
                self.model,
                phase,
                custom_input_prompt_masks=custom_input_prompt_masks,
            )
