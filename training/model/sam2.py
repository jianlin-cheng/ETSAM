...
class SAM2Train(SAM2Base):
    ...
    def forward(self, input: BatchedVideoDatapoint):
        ...
        if input.prompts is None:
            backbone_out = self.prepare_prompt_inputs(backbone_out, input)
        else:
            backbone_out = self.use_prompts_from_dataloader(backbone_out, input)
    ...
    def use_prompts_from_dataloader(self, backbone_out, input, start_frame_idx=0):
        """
        Prepare input mask, point or box prompts. Optionally, we allow tracking from
        a custom `start_frame_idx` to the end of the video (for evaluation purposes).
        """
        ...
        prompt_masks_per_frame = {
            stage_id: masks.unsqueeze(1)  # [B, 1, H_im, W_im]
            for stage_id, masks in enumerate(input.prompts)
        }
        # gt_masks_per_frame = input.masks.unsqueeze(2) # [T,B,1,H_im,W_im] keep everything in tensor form
        backbone_out["gt_masks_per_frame"] = gt_masks_per_frame
        num_frames = input.num_frames
        ...
        init_cond_frames = list(range(start_frame_idx, num_frames))
        backbone_out["init_cond_frames"] = init_cond_frames
        backbone_out["frames_not_in_init_cond"] = [
            t for t in range(start_frame_idx, num_frames) if t not in init_cond_frames
        ]
