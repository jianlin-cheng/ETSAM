...
class TomoDatasetWithPrompt(VisionDataset):
    def __init__(
        self,
        transforms,
        training: bool,
    ...
    def _get_datapoint(self, idx):
        ...
        for retry in range(MAX_RETRIES):
            try:
                if isinstance(idx, torch.Tensor):
                    idx = idx.item()
                # sample a video
                video, segment_loader, prompt_segment_loader = self.video_dataset.get_video(idx)
        ...
        datapoint = self.construct(video, sampled_frms_and_objs, segment_loader, prompt_segment_loader)
        for transform in self._transforms:
            datapoint = transform(datapoint, epoch=self.curr_epoch)
        return datapoint

    def construct(self, video, sampled_frms_and_objs, segment_loader, prompt_segment_loader):
        """
        ...
        images = []
        rgb_images = load_npy(sampled_frames)
        # Iterate over the sampled frames and store their rgb data and object data (bbox, segment)
        for frame_idx, frame in enumerate(sampled_frames):
            w, h = rgb_images[frame_idx].shape[1], rgb_images[frame_idx].shape[2]
            images.append(
                Frame(
            ...
            # We load the prompt segments associated with the current frame
            if isinstance(prompt_segment_loader, JSONSegmentLoader):
                prompt_segments = prompt_segment_loader.load(
                    frame.frame_idx, obj_ids=sampled_object_ids
                )
            else:
                prompt_segments = prompt_segment_loader.load(frame.frame_idx)
            for obj_id in sampled_object_ids:
                # Extract the segment
                if obj_id in prompt_segments:
                    assert (
                        prompt_segments[obj_id] is not None
                    ), "None targets are not supported"
                    # segment is uint8 and remains uint8 throughout the transforms
                    prompt_segment = prompt_segments[obj_id].to(torch.uint8)
                else:
                    # There is no target, we either use a zero mask target or drop this object
                    if not self.always_target:
                        continue
                    prompt_segment = torch.zeros(h, w, dtype=torch.uint8)

                if images[frame_idx].prompts is None:
                    images[frame_idx].prompts = []

                images[frame_idx].prompts.append(
                    Object(
                        object_id=obj_id,
                        frame_index=frame.frame_idx,
...
class TomoDataset(VisionDataset):
    def __init__(
        self,
        transforms,
        training: bool,
    ...
    for frame in frames:
        if frame.data is None:
            ...
            with g_pathmgr.open(path, "rb") as fopen:
                # Convert numpy array to float32 before converting to tensor
                numpy_data = np.load(fopen).astype(np.float32)
                all_images.append(torch.from_numpy(numpy_data).permute(2, 0, 1))
            cache[path] = len(all_images) - 1
        else:
            # The frame rgb data has already been loaded
            all_images.append(frame.data)
